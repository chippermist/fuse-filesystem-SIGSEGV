...
// 		memoryBlockManager.get(disk_block_indexber, block);
...
// 	uint16_t char_index = index * block_indexBER_BYTES;
// 	char block_indexber_content[block_indexBER_BYTES];
// 	for (uint8_t i = 0; i < block_indexBER_BYTES; i++)
// 	{
// 		block_indexber_content[i] = block.data[char_index + i];
// 	}
// 	Block::ID disk_block_indexber;
// 	memcpy(&disk_block_indexber, block_indexber_content, sizeof(uint64_t));
